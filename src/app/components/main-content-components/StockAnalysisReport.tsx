      ...
      <div className="bg-[var(--bg-card-primary)] rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-[var(--bg-card-secondary)] rounded-lg p-4">
            ...
            <div className={`text-lg font-semibold ${getRecommendationColor(data.ai_insights?.recommendation?.action || 'Buy')}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[var(--bg-card-secondary)] rounded-lg p-4">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`font-medium ${getActionColor((data.technicals?.summary.moving_averages.overall) || 'Buy')}`}>
          ...
          <div className="bg-[var(--bg-card-secondary)] rounded-lg p-4">
            ...
            <div className="flex items-center justify-between mb-2">
              <span className="text-[var(--text-secondary)]">Overall Signal</span>
              <span className={`font-medium ${getActionColor(data.technicals?.summary?.oscillators?.overall || 'Buy')}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="text-[var(--positive)]">
                  {(((data.analysis?.analyst_ratings?.price_target_avg || 10 )/ (data.overview?.current_price || 1) - 1) * 100).toFixed(2)}%
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {data.analysis?.analyst_ratings?.ratings?.slice(0, 5).map((rating, index) => (
                <div key={index} className="flex items-center justify-between text-sm">
                  ...
                  <div className="text-right">
                    <div className={`font-medium ${getRatingColor(rating.rating || 'hold')}`}>
                      {rating.rating}
                    </div>
                    {rating.price_target && (
              ...
              <tbody>
                {Object.entries(data.analysis?.earnings_estimates || {}).map(([period, estimate]) => (
                  <tr key={period} className="border-b border-[var(--border)]">
                    ...
                    <td className="px-3 py-2">{formatCurrency(estimate.avg_estimate || 153)}</td>
                    <td className="px-3 py-2">{formatCurrency(estimate.high_estimate || 123)}</td>
                    <td className="px-3 py-2">{formatCurrency(estimate.low_estimate || 125)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
