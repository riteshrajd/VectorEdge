...
async function extractTextWithRetry(page: Page, retries = 1) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (i === retries - 2) await page.reload({ waitUntil: 'networkidle2', timeout: 10000 }); // Reload before last retry
      if (i === retries - 1) throw error;
...
async function parseTextWithRetry(cleanedText: string, retries = 1) {
  for (let i = 0; i < retries; i++) {
    try {
      const jsonData = await parseYahooOverview(cleanedText);
      return jsonData
    } catch (error: unknown) {
      if (i === retries - 1) throw error;
